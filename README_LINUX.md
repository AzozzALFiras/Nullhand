...
| Provider | `ai_provider` value | Requires API key | Notes |
|---|---|---|---|
| Anthropic Claude | `claude` | Yes | Set `ai_api_key` |
| OpenAI | `openai` | Yes | Set `ai_api_key`; optional `ai_base_url` for proxies |
| Google Gemini | `gemini` | Yes | Set `ai_api_key` |
| DeepSeek | `deepseek` | Yes | Set `ai_api_key` |
| Grok (xAI) | `grok` | Yes | Set `ai_api_key` |
| Ollama (local LLM) | `ollama` | No | Set `ai_base_url` (default: localhost) and `ai_model` |
| Built-in rule-based | `local` | No | Zero cost, zero external dependency |

---

## Local AI Setup

The `local` provider is a built-in rule-based intent parser. It requires no API key, no network, and no external process. Use it to get started immediately or in environments without internet access.
...
For a local LLM, install [Ollama](https://ollama.com) and pull a model:
